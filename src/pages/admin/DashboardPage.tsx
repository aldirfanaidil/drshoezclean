...
<<<<<<< HEAD
      ...
      if (selectedMonth !== null || selectedYear !== null) {
        if (selectedMonth !== null && orderDate.getMonth() !== selectedMonth) return false;
        if (selectedYear !== null && orderDate.getFullYear() !== selectedYear) return false;
      } else {
        // Use relative period filter only when no month/year is selected
...
<<<<<<< HEAD
  ...
  const shoeServiceStats = useMemo(() => {
    ...
    filteredOrders.forEach((order) => {
      order.shoes.forEach((shoe) => {
        switch (shoe.service) {
          case "DEEP_CLEAN_REGULER":
            stats.reguler++;
            break;
          case "DEEP_CLEAN_EXPRESS":
          case "FAST_CLEAN_EXPRESS":
            stats.express++;
            break;
          case "UNYELLOWING":
            stats.unyellowing++;
  ...
  const shoeServiceChartData = useMemo(() => [
    { name: "Reguler", count: shoeServiceStats.reguler, fill: "hsl(var(--primary))" },
    { name: "Express", count: shoeServiceStats.express, fill: "hsl(var(--info))" },
    { name: "Unyellowing", count: shoeServiceStats.unyellowing, fill: "hsl(var(--warning))" },
    { name: "Recolour", count: shoeServiceStats.recolour, fill: "hsl(var(--success))" },
      ...
      <Card className="opacity-0 animate-fade-in" style={{ animationDelay: "0.7s" }}>
        <CardHeader>
          <CardTitle className="text-lg font-semibold flex items-center gap-2">
            ...
            Statistik Sepatu Masuk per Layanan
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
            {shoeServiceChartData.map((item) => (
              <div key={item.name} className="p-4 rounded-xl border border-border text-center">
          ...
          {shoeServiceChartData.some((d) => d.count > 0) && (
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={shoeServiceChartData}>
                  <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                  <XAxis
                    dataKey="name"
                  ...
                  <Tooltip
                    ...
                    formatter={(value: number) => [value, "Sepatu"]}
                  />
                  <Bar dataKey="count" radius={[4, 4, 0, 0]}>
                    {shoeServiceChartData.map((entry, index) => (
