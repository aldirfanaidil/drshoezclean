...
export const useAppStore = create<AppState>()(persist((set, get) => ({
  ...
  updateUser: async (id, updates) => {
    ...
    set((state) => ({
      users: state.users.map((u) =>
        u.id === id ? { ...u, ...finalUpdates } : u
      ),
    }));

    const dbUpdates: any = {};
    if (updates.username !== undefined) dbUpdates.username = updates.username;
    if (hashedPassword) dbUpdates.password = hashedPassword;
  ...
  resetAllData: async () => {
    ...
    try {
      // Delete all orders (use gt with null UUID to match all)
      await supabase.from("orders").delete().gt("id", "00000000-0000-0000-0000-000000000000");

      // Delete all customers
      await supabase.from("customers").delete().gt("id", "00000000-0000-0000-0000-000000000000");

      // Delete all cash flows
      await supabase.from("cash_flows").delete().gt("id", "00000000-0000-0000-0000-000000000000");

      // Delete all discounts
      await supabase.from("discounts").delete().gt("id", "00000000-0000-0000-0000-000000000000");

      // Delete all branches
      await supabase.from("branches").delete().gt("id", "00000000-0000-0000-0000-000000000000");
